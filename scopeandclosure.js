//Declare two variables, a and b in the outer function's scope and set them to a string and an
// object respectively. Log their values immediately.
function outer() {
  let a = "Falcons";
  let b = { player: " Drake London " };
  console.log(a, b);
  //Log the values of a and b in the inner function.
  //Update the inner function to take two parameters named a and b.
  function inner(a, b) {
    console.log(a, b);
    a = "Atlanta";
    b.player = " Jessie Bates "; //Inside the inner function, update a property of the b object.
    console.log(a, b);
    //Inside the inner function, assign new values to a and b and log them at the end of the function
    // AND after the execution of inner(a,b).
  }
  //Pass a and b in as arguments when you execute inner().
  inner(a, b);
  console.log(a, b);
}

outer();
